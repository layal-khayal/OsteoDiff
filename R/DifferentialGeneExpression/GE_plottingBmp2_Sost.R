...
# ------------ Bglap2
Bglap <- plotCounts(ddseq, gene= "Bglap2", intgroup = "condition", normalized = TRUE, transform = FALSE, returnData = TRUE)
BglapMean <- plyr::ddply(Bglap, "condition", plyr::summarise, mean = mean(count))
g5 <- ggplot(Bglap, aes(x=condition, y=count))+ 
  geom_point(shape=17,colour="blue", size=3, position = position_jitter(height = 0.01, width = 0.1) )+
  geom_point(data = BglapMean, aes(x= condition, y = mean),shape= 4 ,colour="red", size = 4)+
  ggtitle("Bglap2")+
  scale_y_continuous(limits=c(0,max(Bglap$count)*1.1),
                     labels = scientific_format(digits = 2) ) +
  theme_Publication()
...
g6<-ggplot(Sost, aes(x=condition, y=count))+ 
  ...
  ggtitle("Sost")+
  scale_y_continuous(limits=c(0,max(Sost$count)*1.1),
                     labels = scientific_format(digits = 2) ) +
  theme_Publication()
...
grid_arrange_shared_legend <- function(..., ncol = length(list(...)), nrow = 1, position = c("bottom", "right"),
  ...
  grid.draw(combined)
  if(!length(labs) == 0){
    grid.text(labs[1], x=labpos[[1]][1], y=labpos[[1]][2], gp=gpar(fontsize=16, fontfamily= 'sans', fontface= c('bold','italic')))
    grid.text(labs[2], x=labpos[[2]][1], y=labpos[[2]][2], rot=90, gp=gpar(fontsize=16, fontfamily= 'sans', fontface= c('bold','italic')))
  }
...
pdf("GE_counts_Bmp2-Sost.pdf", width = 6.68, height = 7.2, pointsize = 16, family = 'sans', onefile=FALSE)
grid_arrange_shared_legend(g1, g2, g3, g4,g5,g6, ncol= 2,nrow = 3, labs=list("Differentiation Time-point", "Expression / normalized counts"))
dev.off()
